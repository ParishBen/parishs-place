const Blog1 = () => {
    return (
        ...
        {(isNaN(window.location.pathname.split('/').slice(-1))) ? '' :
              <button id="prev" style={{position:'fixed',left:'2pt'}} onClick={()=>window.location.pathname = "/blogs/1"}>Previous Blog</button>}
             {(isNaN(window.location.pathname.split('/').slice(-1))) ? '' :
              <button id="next" style={{position:'fixed',right:"2pt"}} onClick={()=>window.location.pathname = "/blogs/3"}>Next Blog</button>}
        </div>
    )
}
